...
class NotSameAsTest extends TestCase
{
    ...
    {
        $op = new NotSameAs(new Variable('a', 1), new Variable('b', 2));
        $context = new Context();
    ...
    public function testComparesUsingLooseEquality(mixed $left, mixed $right, bool $strict, bool $loose): void
    {
        $op = new NotSameAs(new Variable('left', $left), new Variable('right', $right));

        self::assertSame(!$loose, $op->evaluate(new Context()));
    }
